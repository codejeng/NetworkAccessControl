...
def add_user(uuid , user_id, first_name, last_name, email, role='student') :
    ...
    try:
        ...
        cursor_row = cursor.execute('''
            SELECT COUNT(*) FROM users_reg 
            WHERE (uuid = ? OR user_id = ? OR email = ?) AND is_deleted = 1
        ''', (uuid, user_id, email)).fetchone()[0]

        if cursor_row > 0:
            cursor.execute('''
                UPDATE users_reg 
                SET user_id = ?, uuid = ?, first_name = ?, last_name = ?, name = ?, email = ?, role = ?, is_deleted = 0, updated_at = CURRENT_TIMESTAMP
                WHERE uuid = ?
            ''', (user_id, uuid, first_name, last_name, first_name + ' ' + last_name, email, role, uuid))
            return
...
def delete_user(id) :
    ...
    try:
        cursor.execute('''UPDATE users_reg SET is_deleted = 1, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?''', (id,))
        conn.commit()
...
def check_is_user_id_exist(user_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute('SELECT 1 FROM users_reg WHERE user_id = ? AND is_deleted  = 0 LIMIT 1', (user_id,))
...
@app.route('/api/delete_user/<string:id>', methods=['GET'])
